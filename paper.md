...
isoform usage. A bundled, download-free self-test reproduces the reference result
on which the tool was developed (short-isoform predominance of the leptin receptor
in human choroid plexus across two independent public cohorts) on a clean machine
in seconds.
...
study's result — short-isoform predominance across two independent public
...
repository runs its analysis through this package, so the result is
